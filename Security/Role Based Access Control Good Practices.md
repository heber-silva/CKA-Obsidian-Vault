...
Permission to create workloads (either Pods, or [workload resources](https://kubernetes.io/docs/concepts/workloads/controllers/) that manage Pods) in a namespace implicitly grants access to many other resources in that namespace, such as [[Secret]]s, [[ConfigMap]]s, and [[PersistentVolume]]s that can be mounted in [[Pod]]s. Additionally, since Pods can run as any [[ServiceAccount]], granting permission to create workloads also implicitly grants the API access levels of any service account in that namespace.
